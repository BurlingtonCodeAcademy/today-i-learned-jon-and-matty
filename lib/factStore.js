...
class FactStore {
  ...
  */
  ...
  async printEntry(index) {
    let collection = await this.collection();
    let fact = await collection.findOne({ "index": index });
    let when = moment(fact.when);
    let output = fact.index + when.format('  hh:mm a - ') + fact.text;
    console.log(output);
  }
  ...
  async editFact(index, text) {

    let collection = await this.collection();
    await collection.findOneAndUpdate({ "index": index }, { $set: { "text": text } });
  }
  ...
  async deleteFact(index) {
    ...
    let collection = await this.collection();
    let entry = collection.findOne({ "index": index });
